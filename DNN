...
def dnn(_X,_weight,_biases,_dropout):
    # 不同层的dropout 可以不同
    _X = tf.nn.dropout(_X,_dropout)
    d1 = tf.nn.relu(tf.nn.bias_add(tf.matmul(_X,_weight['wd1']),_biases['bd1']),name= "d1")
    
    d2x = tf.nn.dropout(d1,_dropout)
    d2 = tf.nn.relu(tf.nn.bias_add(tf.matmul(d2x,_weight['wd2']),_biases['bd2']),name= "d2")
    
    dout = tf.nn.dropout(d2,_dropout)
    out = tf.matmul(dout,_weight['out']) + _biases['out']
    return out
weights = {
